...
module _ {a p q} {A : Set a} (P : A → Set p) (Q : A → Set q)
  ...
  fibers-total {xv} = (iso h g h-g g-h)
   where
    ...
    g-h {xv} ((a , p) , eq) = J (λ _ eq₁ → g (h ((a , p) , eq₁)) ≡ ((a , p) , eq₁)) ((cong g (JRefl (λ xv₁ eq₁ → fiber (f (xv₁ .fst)) (xv₁ .snd)) (p , refl)))
                                    )
                                 (eq)
  -- half of Thm 4.7.7
...
module ContrToUniv {ℓ : Level} {U : Set ℓ} {ℓr} (_~_ : U → U → Set ℓr)
  ...
  lemma : ∀ {A B} → isEquiv (idTo~ {A} {B})
  lemma {A} {B} = fiberEquiv (λ z → A ≡ z) (λ z → A ~ z) (\ B → idTo~ {A} {B})
                  (λ { .equiv-proof y → isContrSigma ((_ , refl) , (\ z → contrSingl (z .snd)))
